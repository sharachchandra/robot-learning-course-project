...
import robomimic.models.transformer_nets as TransformerNets
...
def algo_config_to_class(algo_config):
    ...
    transformer_enabled = ("trans" in algo_config and algo_config.trans.enabled)
...
class BC_Transformer(BC):
    """
    BC training with an Transformer policy.
    """
    def _create_networks(self):
        """
        Creates networks and places them into @self.nets.
        """
        self.nets = nn.ModuleDict()
        self.nets["policy"] = TransformerNets.TransformerActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self.ac_dim,
            mlp_layer_dims=self.algo_config.actor_layer_dims,
    ...
    def reset(self):
        ...
        Reset algo state to prepare for environment rollouts.
        """
        self.nets["policy"].reset_history()
...
class BC_Transformer_GMM(BC_Transformer):
    ...
    def _create_networks(self):
        ...
        """
        assert self.algo_config.gmm.enabled
        assert self.algo_config.trans.enabled

        self.nets = nn.ModuleDict()
        self.nets["policy"] = TransformerNets.TransformerGMMActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self.ac_dim,
            mlp_layer_dims=self.algo_config.actor_layer_dims,
