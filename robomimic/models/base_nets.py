...
class VisionTransformer(ConvBase):
    """
    A VisionTransformer block that can be used to process input images.
    """
    def __init__(
        self,
        pretrained=False,
        ...
        self.net = vision_models.vit_b_16(pretrained=pretrained)

        # cut the last fc layer
        self.nets = torch.nn.Sequential(*(list(self.net.children())[:-1]))
        self.conv = self.nets[0] 
        self.encoder = self.nets[1]
    ...
    def forward(self, inputs):
        ...
        x = self.net._process_input(inputs)
